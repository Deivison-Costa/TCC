...
Como destacado por \textcite{yang_nature2023}, tem-se observado uma tendência multi-decadal de aceleração no aumento da ET desde a década de 1980. Essas mudanças estão intimamente ligadas ao crescimento da vegetação e da área plantada, como evidenciado pelo aumento do índice de área foliar (LAI). Eles indicam que a mensuração da ET é uma prática essencial para avaliar os impactos das atividades antropogênicas no ciclo hidrológico e no clima.
...
A evapotranspiração de referência ($ETo$) é determinada por fatores como radiação líquida ($R_n$), calor do solo ($G$), déficit de pressão de vapor ($\Delta$), constante psicrométrica ($\gamma$), temperatura média do ar ($T$), velocidade do vento a 2 metros de altura ($u2$), pressão de vapor do ar saturado ($e_s$) e pressão de vapor do ar ($e_a$).
...
A determinação da ETo em tempo real é um dos principais objetos de estudo dessa pesquisa. Na próxima seção, serão apresentados trabalhos similares e como a proposta deste trabalho se diferencia deles.