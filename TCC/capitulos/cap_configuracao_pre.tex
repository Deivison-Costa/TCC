...
A evapotranspiração (ET) descreve o processo fundamental de transferência de água da superfície da Terra para a atmosfera \cite{carmody_fao2023}. Como ilustrado no ciclo hidrológico (Figura \ref{figura:ciclo_agua}), a ET desempenha um papel essencial na dinâmica hídrica. Foi observado que:
...
A ET pode ser estimada por meio do modelo de Penman-Monteith, que é amplamente utilizado para calcular a ET de referência (ETo) em condições padrão. O modelo de Penman-Monteith é definido pela equação apresentada por \textcite{Allen_evapotranspiration1998}:

\begin{equation}
ET = \frac{\Delta (R_n - G) + \gamma \cdot \frac{C_n}{C_n + \gamma} \cdot (e_s - e_a)}{\Delta + \gamma \cdot (1 + \frac{C_d}{C_n})}
\end{equation}

Onde:
\begin{itemize}
  \item \(ET\) é a taxa de ET (mm/dia)
