...
O sensor DHT22 (Figura \ref{figura:dht22}) mede a temperatura e a umidade do ar, com boa precisão e um intervalo de operação amplo \parencite{DHT22}. O pino de saída do sensor foi conectado ao pino GPIO 4 do ESP32, configurado no código como \texttt{DHTPIN}. Para alimentação, o sensor foi conectado ao pino 3V3 e ao GND do ESP32. Na Tabela \ref{tab:dht22} são apresentadas as especificações técnicas do sensor.

O uso do sensor DHT22 no sistema justifica-se pela sua alta precisão e ampla faixa de operação, características fundamentais para o monitoramento climático \parencite{DHT22}. A medição precisa de temperatura e umidade relativa do ar é indispensável em aplicações meteorológicas, pois esses parâmetros influenciam diretamente fenômenos atmosféricos e condições ambientais. Além disso, a facilidade de integração ao ESP32 por meio de um único pino digital reduz a complexidade do circuito, otimizando a implementação do sistema. A escolha do DHT22, em detrimento de outros sensores, também considera o seu equilíbrio entre custo-benefício e desempenho, sendo amplamente utilizado em projetos de monitoramento ambiental \parencite{ferrandez2018precision,Francisco_precision2024}.
...
O sensor BMP280 (Figura \ref{figura:bmp280}) mede a pressão atmosférica com alta precisão, além de oferecer medições de temperatura \parencite{BMP280}. Ele utiliza o barramento I2C, sendo conectado aos pinos padrão do ESP32 para \texttt{SDA} e \texttt{SCL}. Para alimentação, o sensor também foi conectado ao pino 3V3 e ao GND do ESP32. Na Tabela \ref{tab:bmp280} são apresentadas as especificações técnicas do sensor.

A inclusão do sensor BMP280 no sistema deve-se à sua capacidade de medir pressão atmosférica com alta precisão, o que é necessário para a análise de condições meteorológicas e a previsão de mudanças climáticas \parencite{BMP280}. A pressão atmosférica é um parâmetro utilizado na modelagem de padrões climáticos, e o BMP280 oferece dados confiáveis dentro de uma faixa de operação ampla, adequados tanto para altitudes elevadas quanto para ambientes ao nível do mar. Além disso, a integração por barramento I2C simplifica a comunicação com o microcontrolador ESP32, permitindo o compartilhamento de pinos com outros dispositivos. A alta resolução do sensor possibilita a detecção de variações sutis na pressão, úteis para aplicações detalhadas, como estudos de microclimas \parencite{Prathibha_system2017, Lamine_precision2024}.
...
O comparador LM393 foi escolhido por ser facilmente ajustável e sensível a variações de umidade no solo \parencite{LM393}. A umidade do solo é um fator importante no contexto da irrigação, influenciando diretamente a absorção de nutrientes pelas plantas \parencite{Zhang_evapotranspiration_med2016}. O LM393 permite a detecção rápida e precisa de mudanças na umidade do solo, tornando a coleta em tempo real mais eficiente e confiável. Na Tabela \ref{tab:lm393} são apresentadas as especificações técnicas do comparador.
...
% Nessa pesquisa foram adotadas medidas para minimizar tais erros antes de integrar os sensores ao ESP32. Para isso, foram realizados testes de calibração e ajustes nos sensores, além de medidas de controle de qualidade para garantir a precisão e a confiabilidade dos dados coletados.
...
Por fim, os componentes foram integrados em um sistema cyber-físico que publica as leituras individuais de cada sensor em um \textit{broker}. Nesse trabalho optou-se pelo EMQX (Figura \ref{figura:emqx}), uma plaforma de código aberto para MQTT de alta performance \parencite{EMQX}.
...
Para o desenvolvimento do sistema proposto neste trabalho, foram empregadas ferramentas e tecnologias. Cada uma delas desempenharam um papel específico e contribuiram para a robustez e a eficiência do projeto.

\subsection{Linguagem \textit{C}}
A linguagem de programação \textit{C} foi utilizada para o desenvolvimento do \textit{firmware} do ESP32. Essa escolha deve-se à sua eficiência e proximidade com o \textit{hardware}, permitindo maior controle sobre os recursos embarcados. Além disso, o \textit{C} é suportado nativamente por diversas plataformas de desenvolvimento, o que facilita a portabilidade do código.
...
\begin{itemize}
    \item \texttt{Arduino}: Biblioteca base para desenvolvimento com a plataforma Arduino, fornecendo funções essenciais para manipulação de pinos, comunicação serial e temporização.
    \item \texttt{ModbusMaster}: Implementação do protocolo Modbus RTU, permitindo comunicação eficiente entre dispositivos escravos e mestres em redes industriais.
    \item \texttt{Wire}: Biblioteca utilizada para comunicação I2C, facilitando a integração com sensores e dispositivos externos.
    \item \texttt{WiFi}: Biblioteca nativa para gerenciar conexões Wi-Fi no ESP32, essencial para conectividade com redes locais e a \textit{internet}.
    ...
    \item \texttt{WiFiClientSecure}: Extensão da biblioteca Wi-Fi para conexões seguras utilizando TLS/SSL, garantindo comunicações criptografadas.
    \item \texttt{PubSubClient}: Implementação do protocolo MQTT, fundamental para a comunicação leve e eficiente com o \textit{backend}.
    \item \texttt{TinyGPSPlus}: Biblioteca para interpretação de dados recebidos de módulos GPS, útil para obter informações de localização geográfica.
\end{itemize}

A combinação dessas bibliotecas permitiu a criação de um \textit{firmware} robusto, capaz de monitorar e controlar diversos sensores e atuadores, além de garantir conectividade segura e comunicação eficiente com sistemas externos.

\subsection{\textit{Node.js}}
O ambiente de execução \textit{Node.js} foi empregado no desenvolvimento do \textit{backend} devido à sua performance e capacidade de gerenciar múltiplas conexões simultâneas de forma eficiente. \textit{Node.js} é particularmente adequado para aplicações que exigem baixa latência, como em sistemas em tempo real.
...
\begin{itemize}
    \item \texttt{Express}: \textit{Framework} leve e flexível para construção de servidores \textit{web}, usado para gerenciar rotas e requisições HTTP.
    \item \texttt{Axios}: Biblioteca utilizada para realizar requisições HTTP de maneira simples e eficiente, facilitando a comunicação com APIs externas.
    \item \texttt{Cors}: \textit{Middleware} que permite configurar políticas de compartilhamento de recursos entre diferentes origens, essencial para integrar o \textit{frontend} com o \textit{backend}.
    \item \texttt{Dotenv}: Biblioteca usada para gerenciar variáveis de ambiente, garantindo segurança e flexibilidade na configuração de credenciais e configurações do sistema.
    \item \texttt{MQTT}: Protocolo leve para comunicação em tempo real, empregado no gerenciamento de mensagens entre dispositivos conectados.
...
\begin{itemize}
    \item \texttt{Typescript}: \textit{Superset} do JavaScript que adiciona tipagem estática ao código, prevenindo erros comuns.
...
\begin{itemize} 
  ...
  \item \texttt{tailwindcss-animate}: Plugin que adiciona animações ao \textit{TailwindCSS}, tornando a experiência do usuário mais fluida e atraente. 
...
As tecnologias e ferramentas mencionadas foram selecionadas para atender aos requisitos específicos do sistema proposto, priorizando desempenho, escalabilidade, confiabilidade e experiência prévia de desenvolvimento.
